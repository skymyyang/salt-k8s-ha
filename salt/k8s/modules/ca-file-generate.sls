...
kubectl-admin-set-cluster:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-cluster kubernetes --certificate-authority=ca.pem --embed-certs=true --server={{ pillar['KUBE_APISERVER'] }} --kubeconfig=kubectl.kubeconfig

kubectl-admin-set-credentials:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-credentials kubernetes-admin --client-certificate=admin.pem --embed-certs=true --client-key=admin-key.pem --kubeconfig=kubectl.kubeconfig

kubectl-admin-set-context:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-context kubernetes-admin@kubernetes --cluster=kubernetes --user=kubernetes-admin --kubeconfig=kubectl.kubeconfig

kubectl-admin-use:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config use-context kubernetes-admin@kubernetes --kubeconfig=kubectl.kubeconfig && mkdir -p ~/.kube && /bin/cp /etc/kubernetes/sslcert/kubectl.kubeconfig ~/.kube/config
...
kube-controller-manager-set-cluster:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-cluster kubernetes --certificate-authority=ca.pem --embed-certs=true --server={{ pillar['KUBE_APISERVER'] }} --kubeconfig=kube-controller-manager.kubeconfig

kubectl-controller-manager-set-credentials:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-credentials system:kube-controller-manager --client-certificate=kube-controller-manager.pem --embed-certs=true --client-key=kube-controller-manager-key.pem --kubeconfig=kube-controller-manager.kubeconfig

kubectl-controller-manager-set-context:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-context system:kube-controller-manager@kubernetes --cluster=kubernetes --user=system:kube-controller-manager --kubeconfig=kube-controller-manager.kubeconfig

kubectl-controller-manager-use:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config use-context system:kube-controller-manager@kubernetes --kubeconfig=kube-controller-manager.kubeconfig
  file.copy:
    - user: root
    - group: root
    - mode: 644
...
kubectl-scheduler-context:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config set-context system:kube-scheduler@kubernetes --cluster=kubernetes --user=system:kube-scheduler --kubeconfig=kube-scheduler.kubeconfig

kubectl-scheduler-use:
  cmd.run:
    - name: cd /etc/kubernetes/sslcert && kubectl config use-context system:kube-scheduler@kubernetes --kubeconfig=kube-scheduler.kubeconfig
  file.copy:
    - user: root
    - group: root
    - mode: 644
